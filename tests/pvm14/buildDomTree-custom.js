...
(
  ...
  console.log(`ARMASPALPUEBLO args.initialRootNode = [${args.initialRootNode}] args.highlightIndex = [${args.highlightIndex}] ...`);
  ...
  console.log(`buildDomTree called with args: ${JSON.stringify(args)} ...`);
  ...
  function getCachedComputedStyle(element) {
    ...
    } else { // PVM14 => https://developer.mozilla.org/en-US/docs/Web/API/Window/getComputedStyle
      style = window.getComputedStyle(element);
    }
  ...
  function highlightElement(element, index, parentIframe = null) {
    ...
    try {
      ...
      if (!container) {
          ...
          container.style.zIndex = "2147483647"; // https://www.w3schools.com/jsref/prop_style_zindex.asp
      ...
      // Calculate position => PVM14 I THINK THAT THIS IS ALL WHAT IS DONE REGARDING THE POSITION AND SIZE OF THE RECTANGLE
      ...
      /* Add to container => PVM14 THIS IS IMPORTANT TOO ... THEY ARE ADDED HERE TO THE 'container'
         AND FROM THIS MOMENT ON, THEY SHOULD BE VISIBLE */
  ...
  function getXPathTree(element, stopAtBoundary = true) {
    ...
    while (currentElement && currentElement.nodeType === Node.ELEMENT_NODE) {
      // Stop if we hit a shadow root or iframe => PVM14 I think this is correct because it marks a 'limit', yes
      if (
        stopAtBoundary &&
      ...
      /* PVM14: This code counts how many preceding sibling elements have the same tag name
         as the currentElement. The index variable stores this count. */
      let index = 0;
  ...
  function isTextNodeVisible(textNode) {
    try {
      /* PVM14: From https://developer.mozilla.org/en-US/docs/Web/API/Document/createRange
  ...
  // Helper function to check if element is accepted => PVM14 What does it mean "to be accepted"?
  ...
  function isElementVisible(element) {
    ...
    return (
        element.offsetWidth > 0 &&       // https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/offsetWidth
        element.offsetHeight > 0 &&      // https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/offsetHeight
        style.visibility !== "hidden" &&
        style.display !== "none"
    );
  }
  ...
  function isTopElement(element) {
    ...
    const isInViewport = ( // => check if the element's bounding rectangle (rect) has any overlap with the browser's viewport
    ...
    let doc = element.ownerDocument; // => https://developer.mozilla.org/en-US/docs/Web/API/Node/ownerDocument
    ...
    const shadowRoot = element.getRootNode(); // => This is supposed to work with both closed and open ...
        ...
        try {
            const topEl = measureDomOperation(
                /* https://developer.mozilla.org/en-US/docs/Web/API/Document/elementFromPoint
                   The elementFromPoint() method, available on the Document object, returns the topmost Element
  ...
  function buildDomTree(node, parentIframe = null) {
    ...
    if (!node || node.id === HIGHLIGHT_CONTAINER_ID) { // => THIS IS THE <div> 'container' ELEMENT PARENT OF ALL HIGHLIGTED RECTANGLES
      if (debugMode) PERF_METRICS.nodeMetrics.skippedNodes++;
      return null;
    }

    // Special handling for root node (body)
    // => PVM14: TO BE CHANGED THIS FUNCTION WILL POTENTIALLY RECEIVE NOT 'body' root ELEMENTS
    ...
    /* TODO:PVM14 tentative: Special handling when the root node is a ShadowRoot which
    ...
    if (viewportExpansion !== -1) {
      ...
      if (!rect || (
          /* PVM14: These lines are checking if the element is completely outside an imaginary box
             that's the viewport expanded by viewportExpansion pixels on all four sides */
          rect.bottom < -viewportExpansion ||
          rect.top > window.innerHeight + viewportExpansion ||
          rect.right < -viewportExpansion ||
          rect.left > window.innerWidth + viewportExpansion
    ...
    /* Process element node => PVM14: YOU ONLY ARRIVE HERE IF:
       node !== document.body && node.nodeType === Node.ELEMENT_NODE && isElementAccepted(node) &&
       the element IS NOT OUSIDE THE VIEWPORT + EXPANSION WINDOW */
    const nodeData = {
    ...
    // Check interactivity => PVM14 I guess this is not too relevant for what i want to achieve ...
        ...
        if (nodeData.isTopElement) {
          ...
          if (nodeData.isInteractive) {
            ...
            if (doHighlightElements) {
              ...
              } else { // If focusHighlightIndex is -1 (the default), all interactive elements are highlighted.
                highlightElement(node, nodeData.highlightIndex, parentIframe);
              }
            }
          }
    ...
    // => PVM14 I think here it is what I will change
    if (node.tagName) {
      ...
      if (tagName === "iframe") {
        try {
          /* PVM14 I KNOW FOR A FACT THAT ACCESS TO IFRAME'S contentDocument IS NOT AVAILABLE UNLESS
            '--disable-site-isolation-trials'. I DON'T KNOW ABOUT node.contentWindow?.document YET, BUT FROM
            https://developer.mozilla.org/en-US/docs/Web/API/HTMLIFrameElement/contentWindow
            I GUESS ACCESS TO THAT IS LIMITED TOO IN THE SAME WAY */
      ...
      // Handle rich text editors and contenteditable elements
        ...
        node.isContentEditable || // https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/isContentEditable
      ...
      /*+ Handle shadow DOM => PVM14: THIS WILL NEVER BE ABLE TO DEAL WITH CLOSED SHADOW ROOTS BECAUSE OF
          THE SIMPLE FACT THAT THEY'RE NOT ACCESSIBLE VIA shadowRoot PROPERTY */
