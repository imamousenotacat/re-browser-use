...
# TODO: PVM14
# I created this class to gather, for the moment, the functionality I'm writing. The methods in this class could probably be
...
class DomUtils:
  def _get_closed_shadow_roots_from_node(self, node: Dict, current_node_xpath: str, results: List[str]) -> List[str]:
    ...
    # Only elements (which have a localName) can host shadow roots. TODO: PVM14 ?
    ...
    if node.get('localName') and node.get('contentDocument'):
      # For content_doc_node, the XPath effectively resets. Its children (e.g. <html>) start from "/". TODO: PVM14 I'M NOT SO SURE
  ...
  def xpath_to_css(self, xpath: str) -> str:
    ...
    for i in range(start_index, len(segments)):
      ...
      if not match:
        # This segment is not in 'tag' or 'tag[index]' format. TODO:PVM14 I don't like it
  ...
  def _get_all_frames_recursively(self, initial_frame: Frame) -> List[Frame]:
    ...
    # TODO: PVM14 THIS IS CRAZY YOU GET MORE THAN 15 FOR https://nopecha.com/demo/cloudflare
  ...
  async def get_insertion_point_for_body(final_dom_element_node: Optional[DOMElementNode], frame : Frame) -> DOMElementNode|None:
    ...
    # TODO: pvm14 The real problem is how to match the iframe_elements with the corresponding Frame object.
